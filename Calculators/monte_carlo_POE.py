...
from simpoe.dents import failureCycles, dentLimitStates
...
def sccpoe(df,n):
    ...
    ruptures, rupture_count = crackLimitStates.ls_scc_rupture(failPress, OP, bulk=True)

    leaks, leak_count = crackLimitStates.ls_scc_leak(WTd, cD, bulk=True)
...
def rdpoe(df, n):
    ...
    NF = failureCycles.nf_EPRG(ODd, WTd, UTSd, dL_run, dW_run, dD_run, gD, MAXStr, MINStr)

    n_cycles = time_delta * cycles

    fails, fail_count = dentLimitStates.ls_dent_fail(NF, n_cycles, bulk=True)
...
if __name__ == '__main__':
    CSV_FILE_NAME = 'crack_poe_inputs.csv'
    ...
    iter = 1_000_000
    order = ['InlineInspectionCrackAnomalyId','PDP_frac', 'clength', 'iterations', 'POE_l', 'POE_r', 'POE']
    order2 = ['InlineInspectionCrackAnomalyId', 'POE_l', 'POE_r', 'POE']
    
    chunks = split(df, 100)
    
    if not('chunks' in locals()):
        result, qc = sccpoe(df,iter)
        end = time.time()
        ...
        result = result.merge(df, on='InlineInspectionCrackAnomalyId')

        print("Stress Corrosion Cracking", "POE Simulation\n")
        print(result[order2], '\n')
        print("Aggregated POE for these features is {}.\n".format(agg_POE))
        print("Calculation took {} seconds.".format(end - start))
    else:
        results = [sccpoe(x,iter)[0] for x in chunks]
